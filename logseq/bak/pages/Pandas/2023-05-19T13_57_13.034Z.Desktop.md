...
- DataFrame
	...
	- 切片操作
		- ```python
		  data=pd.DataFrame(np.arange(12).reshape(3,4),columns=list('abcd'),index=list('abc'))
		  data.iloc[:,0]
		  data.loc['a':'c','a':'c']
		  ```
		- |  | a | b | c |
...
- 注意:区分两个dataframe
	- ```python
	  pd.DataFrame([1,2,3],[2,3,4])与pd.DataFrame([[1,2,3],[2,3,4]])
	  ```
-